...
func (op *Operator) MatrixLoader(frames []*Frame) LoadFunc {
	...
	matrixDatas := GetMatrixDatasAfter(op.Name, frames[0].Time)
...
func (op *Operator) Execute() {
	// compute rerun time as minimum rerun time of our parents
	// if we have look-behind, then we add in frames from before rerun time
	rerunTime := *(op.Parents[0].RerunTime)
	for _, parent := range op.Parents {
		if rerunTime.After(*parent.RerunTime) {
			rerunTime = *parent.RerunTime
		}
	}
	startTime := rerunTime
	if op.LookBehind > 0 {
	...
	rows := db.Query("SELECT id, video_id, idx, time, bounds FROM video_frames WHERE time >= ? ORDER BY time", startTime)
	...
	for _, frame := range frames {
		if !frame.Time.Before(rerunTime) {
			rerunFrame = frame
			break
		}
	}
